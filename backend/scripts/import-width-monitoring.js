/**
 * Import IT-PHY-001 width monitoring data from exported JSON.
 * Run: cd backend && node scripts/import-width-monitoring.js
...
const DATA_FILE = path.join(__dirname, 'data', 'width-monitoring.json');
const LSL = -0.13;
const USL =  0.13;
...
async function main() {
  ...
  const [[def]] = await sequelize.query(
    `SELECT id FROM test_definitions WHERE test_id = 'IT-PHY-001' LIMIT 1`
  );
  if (!def) throw new Error('IT-PHY-001 not found — run migration 026 first.');
  console.log(`✅ IT-PHY-001 definition id = ${def.id}`);

  let inserted = 0, skipped = 0, entryCount = 0;
  const now = new Date().toISOString();

  for (const rec of records) {
    const sessionNum = `W-${rec.jobNumber}-${rec.testDate}`;
    ...
    for (const card of rec.cards) {
      const pass = card.widthMm >= LSL && card.widthMm <= USL;
      ...
      `, {
        replacements: { sessionId, defId: def.id, cardId: sc.id, value: card.widthMm, pass, now },
      });
      entryCount++;
    }
