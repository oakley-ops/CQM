/**
 * Import IT-PHY-007 height monitoring data from exported JSON.
 * Run: cd backend && node scripts/import-height-monitoring.js
...
const DATA_FILE = path.join(__dirname, 'data', 'height-monitoring.json');
const LSL = -0.13;
const USL =  0.13;
...
async function main() {
  ...
  const [[def]] = await sequelize.query(
    `SELECT id FROM test_definitions WHERE test_id = 'IT-PHY-007' LIMIT 1`
  );
  if (!def) throw new Error('IT-PHY-007 not found — run migration 026 first.');
  console.log(`✅ IT-PHY-007 definition id = ${def.id}`);

  let inserted = 0, skipped = 0, entryCount = 0;
  const now = new Date().toISOString();

  for (const rec of records) {
    const sessionNum = `H-${rec.jobNumber}-${rec.testDate}`;
    ...
    for (const card of rec.cards) {
      const pass = card.heightMm >= LSL && card.heightMm <= USL;
      ...
      `, {
        replacements: { sessionId, defId: def.id, cardId: sc.id, value: card.heightMm, pass, now },
      });
      entryCount++;
    }
