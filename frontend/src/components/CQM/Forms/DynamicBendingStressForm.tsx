...
interface DynamicBendingStressFormProps {
  def: TestDefinition;
...
interface DynamicBendingStressExtra {
  bendingMachineId?: string;
  bendingMachineCalUntil?: string;
...
const DynamicBendingStressForm: React.FC<DynamicBendingStressFormProps> = ({
  ...
  const extra = (meta.extraData ?? {}) as DynamicBendingStressExtra;

  const updateMeta = (patch: Partial<TestEntryMetadata>) => {
    onUpdateEntry(def.id, { specializedMetadata: { ...meta, ...patch } });
  };

  const updateExtra = (patch: Partial<DynamicBendingStressExtra>) => {
  ...
  const cyclesStatus = !cyclesValid
    ? null
    : (cyclesNum as number) < 2000
    ? 'error'
    : (cyclesNum as number) < 4000
    ? 'warning'
    : 'success';
  ...
  return (
    <Box>
      <Typography variant="subtitle2" fontWeight="bold" sx={{ mb: 0.5 }}>
        Dynamic Bending Stress Test Log — ISO/IEC 10373-1 (#8140#)
      </Typography>
      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1.5 }}>
        Test method #8140# · Required: ≥ 2000 cycles (#3042#) · Recommended: 4000–6000 cycles
        &nbsp;·&nbsp; Qualification min. sample size: 8 cards
      </Typography>

      <Alert severity="info" icon={<InfoIcon />} sx={{ mb: 2 }}>
        <Typography variant="caption">
          <strong>ISO 10373-1 note:</strong> The test method for Dynamic Bending Stress was changed
          substantially in the 2020 edition of ISO/IEC 10373-1. Verify your procedure references the
          correct edition. Results are considered independent of artwork — sampling shall consider
          different card constructions (layers, materials, ICM, process flow, and parameters).
        </Typography>
      </Alert>
      ...
      <Grid container spacing={2} sx={{ mb: 2 }}>
        ...
        <Grid item xs={12} sm={6} md={4}>
          <TextField
            label="Bending Machine ID#" size="small" fullWidth
            value={extra.bendingMachineId ?? ''}
            onChange={e => updateExtra({ bendingMachineId: e.target.value })}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={4}>
          <TextField
            label="Machine Cal. Valid Until" size="small" fullWidth type="date"
            value={extra.bendingMachineCalUntil ?? ''}
            onChange={e => updateExtra({ bendingMachineCalUntil: e.target.value })}
            InputLabelProps={{ shrink: true }}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={4}>
      ...
      <Grid container spacing={2} sx={{ mb: 2 }}>
        <Grid item xs={12} sm={6} md={4}>
          <TextField
            ...
            helperText="Required: ≥ 2000 (#3042#) | Recommended: 4000–6000"
            error={cyclesStatus === 'error'}
          />
          {cyclesStatus === 'error' && (
            <Typography variant="caption" color="error">
              Below required minimum of 2000 cycles (#3042#)
            </Typography>
          )}
          {cyclesStatus === 'warning' && (
            <Typography variant="caption" color="warning.main">
              Meets minimum — CQM recommends testing up to 4000–6000 cycles
            </Typography>
          )}
          {cyclesStatus === 'success' && (
            <Typography variant="caption" color="success.main">
              Meets recommended cycle target
            </Typography>
          )}
        </Grid>
        <Grid item xs={12} sm={8}>
          <TextField
            ...
            helperText="Layers, materials, ICM, process flow — artwork not required"
          />
        </Grid>
      </Grid>
      ...
      <Alert severity="info" sx={{ mb: 2, py: 0.5 }}>
        <Typography variant="caption">
          <strong>Pass criteria (#3042#):</strong> No cracks on surface AND card remains fully functional
          after ≥ 2000 bending cycles. Both visual and functional tests must pass for each card.
          &nbsp;
          <strong>Recommended:</strong> Test beyond 2000 cycles (e.g. 4000–6000). Vendor shall retain
          results for all cycle counts and make them available upon request.
        </Typography>
      </Alert>
      ...
      <TextField
        ...
        helperText="Include card construction details (layers, materials, ICM, process flow) if not referenced above."
      />
    </Box>
  );
};

export default DynamicBendingStressForm;
